<div>
    <div class="w-full flex pb-10">
        <div class="w-3/6 mx-1">
            <input wire:model.debounce.300ms="search" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"placeholder="Search starships...">
        </div>
        <div class="w-1/6 relative mx-1">
            <select wire:model="orderBy" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                ...
                <option value="max_speed">M. Speed</option>
                <option value="crew">Crew</option>
                <option value="pasengers">Pasengers</option>
                <option value="cargo_capacity">Cargo Capacity</option>
                <option value="consumables">Consumables</option>
                <option value="hyperdrive_rating">HD. Rating</option>
                <option value="MGLT">MGLT</option>
    ...
    <table class="table-auto w-full mb-6 mt-16">
        <thead>
            <tr>
                ...
                <th class="px-4 py-2">Consumables</th>
                <th class="px-4 py-2">HD. Rating</th>
                <th class="px-4 py-2">MGLT</th>
        ...
        <tbody>
            @foreach($starships as $starship)
                <tr>
                    ...
                    <td class="border px-4 py-2">{{ $starship->length }}</td>
                    ...
                    <td class="border px-4 py-2">{{ $starship->consumables }}</td>
                    <td class="border px-4 py-2">{{ $starship->hyperdrive_rating }}</td>
                    <td class="border px-4 py-2">{{ $starship->MGLT }}</td>
